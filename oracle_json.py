...
class Checks(object):
    def tablespace(self):
	""" Tablespace Used - JSON """
	sql = "SELECT  (select distinct NAME from v$database) as DB, ROUND ( ( ( datafile.bytes  - NVL ( freespace.bytes, 0 ) ) / datafile.bytes ) * 100  ) USED , ts.tablespace_name as TABLESPACE FROM dba_tablespaces ts  , (SELECT tablespace_name      , SUM ( bytes ) bytes      FROM dba_free_space      GROUP BY tablespace_name    ) freespace  , (SELECT   COUNT ( 1 ) datafiles      , SUM ( bytes ) bytes      , tablespace_name      FROM dba_data_files      GROUP BY tablespace_name    ) datafile  WHERE freespace.tablespace_name (+) = ts.tablespace_name  AND datafile.tablespace_name (+)   = ts.tablespace_name and  datafile.bytes is not null   and ts.tablespace_name not in (select distinct value from gv$parameter where name='undo_tablespace')  ORDER BY NVL ( ( ( datafile.bytes - NVL ( freespace.bytes, 0 ) ) / datafile.bytes ), 0 ) DESC"
        self.cur.execute(sql)
        res = self.cur.fetchall()
        key = ['{#DB}','{#USED}','{#NAME}']
        #key = ['{#DATABASE}','{#TABLESPACE}','{#USED}']
        lst = []
        for i in res:
            d = dict(zip(key, i))
            lst.append(d)
    ...
    def asm(self):
        """ ASM - JSON """
        sql = "SELECT NAME, ROUND((((TOTAL_MB - FREE_MB) * 100) / TOTAL_MB))  , CASE WHEN ROUND((((TOTAL_MB - FREE_MB) * 100) / TOTAL_MB)) BETWEEN 0 and 50   THEN 'OK'  WHEN ROUND((((TOTAL_MB - FREE_MB) * 100) / TOTAL_MB)) BETWEEN 51 and 80  THEN 'WARNING'  		WHEN ROUND((((TOTAL_MB - FREE_MB) * 100) / TOTAL_MB)) BETWEEN 81 and 98  THEN 'CRITICAL'  WHEN ROUND((((TOTAL_MB - FREE_MB) * 100) / TOTAL_MB)) BETWEEN 99 and 100 THEN 'TRAGEDY'  		end  FROM V$ASM_DISKGROUP "
        self.cur.execute(sql)
        res = self.cur.fetchall()
        key = ['{#DISKGROUP}','{#PCT}','{#STATUS}']
        lst = []
        for i in res:
            d = dict(zip(key, i))
            lst.append(d)
